...
export const isFreeTextTokenStatus = (
  status: unknown,
): status is ITokenResult["status"] => {
...
export const isFreeTextMatchedRule = (
...
export const isFreeTextValidationSummary = (
  data: unknown,
): data is IFreeTextValidationSummary => {
  if (!isRecord(data)) {
    return false;
  }

  const hasValidStatus =
    data.status === null || isFreeTextTokenStatus(data.status);
...
export const isFreeTextValidationResult = (
  data: unknown,
): data is IFreeTextValidationResult => {
  if (!isRecord(data)) {
    return false;
  }

  const hasValidTokens =
    Array.isArray(data.tokens) && data.tokens.every(isFreeTextTokenResult);
  const hasValidSegments =
    Array.isArray(data.segments) && data.segments.every(isFreeTextSegment);
...
export const isFreeTextSuggestResponse = (
  data: unknown,
): data is ISuggestResponse => {
  if (!isRecord(data)) {
    return false;
  }
